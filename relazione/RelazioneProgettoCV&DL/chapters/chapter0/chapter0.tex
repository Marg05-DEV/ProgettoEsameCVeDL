\graphicspath{{chapters/chapter0/img}}

\begin{itemize}
    \item $\eps > 0$ \hfill (\texttt{\textbackslash eps})
    \item $\veps > 0$ \hfill (\texttt{\textbackslash veps})
    \item $\opensets$ \hfill (\texttt{\textbackslash opensets})
    \item $\oset$ \hfill (\texttt{\textbackslash oset})
    \item $\opset{1}$ \hfill (\texttt{\textbackslash opset})
    \item $\lub(A)$ \hfill (\texttt{\textbackslash lub})
    \item $\del{f}{x}$ \hfill (\texttt{\textbackslash del})
    \item $\Del{2}{f}{x}$ \hfill (\texttt{\textbackslash Del})
    \item $\deld{f}{x}$ \hfill (\texttt{\textbackslash deld})
    \item $\Deld{2}{f}{x}$ \hfill (\texttt{\textbackslash Deld})
    \item $\der{f}{x}$ \hfill (\texttt{\textbackslash der})
    \item $\lm \in \mathbb{R}$ \hfill (\texttt{\textbackslash lm})
    \item $x \uin A$ \hfill (\texttt{\textbackslash uin})
    \item $A \usubset B$ \hfill (\texttt{\textbackslash usubset})
    \item $\lt( \frac{1}{2} \rt)$ \hfill (\texttt{\textbackslash lt, \textbackslash rt})
    \item $( \frac{1}{2} )$ \hfill (\texttt{no \textbackslash lt, \textbackslash rt})

    \item $\bs{v}$ \hfill (\texttt{\textbackslash bs})
    \item $\exs x \in A$ \hfill (\texttt{\textbackslash exs})
    \item $\dps{\sum_{i=0}^n i}$ \hfill (\texttt{\textbackslash dps})
    \item $\id_A$ \hfill (\texttt{\textbackslash id})
\end{itemize}
